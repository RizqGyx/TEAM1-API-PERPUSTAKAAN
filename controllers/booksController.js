  ...
  try {
    ...
    if (req.user.role == "Staff") {
      if (checkRack.libraryId !== req.user.libraryId) {
        return next(
          new apiError("Sorry but you dont have access in this library")
        );
      }
    }
...
const updateBookData = async (req, res, next) => {
  try {
    ...
    if (checkId.Rack.id !== req.user.libraryId)
      return next(
        new apiError(`Sorry but you dont have access in this library`, 400)
      );
    ...
    const updatedBookData = await updateData(
      req.body,
      uploadedImageUrls,
      checkId
    );
...
const deleteBookById = async (req, res, next) => {
  try {
    ...
    if (book.Rack.id !== req.user.libraryId)
      return next(
        new apiError(`Sorry but you dont have access in this library`, 400)
      );
